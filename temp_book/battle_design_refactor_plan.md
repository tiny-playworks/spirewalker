...
- Act 1 首精英当前设计状态（基线）：
  - 三只精英职责分工已稳定：
    - `executioner`：反制 + 检定 + 后续追压（已将 heavy 后移一拍）
    - `twin_hunter`：多段输出 + 惩罚多打（主要压制 burst）
    - `debt_monk`：资源干扰 + 手牌压力（主要压制不成型构筑）
  - 已验证：
    - executioner 的问题主要来自“节奏过早”，而非单点数值
    - 通过将 rotation 调整为 `counter -> attackBuff -> heavy`，显著改善 guard 通过率且无副作用
    - 当前不再继续通过单参数（heavy / vulnerable / counter）微调
...
- starter 改为“能理解机制，但不能稳定滚雪球”的 10 张固定表：
  ...
  - `1x brace_rhythm`
  - `1x measured_rest`
- Act 1 guard 路线专项支持（当前基线）：
  - `anchor_slash`：承压回合推进（攻防一体 + 稳势）
  - `stable_mind`：回合质量修复（防守 + 手牌续航）
  - 结论：
    - 两张牌已覆盖“承压推进 + 回合稳定”两类能力
    - 后续不再通过继续新增 guard 专项牌解决首精英问题
- momentum 开局规则明确为：
  - 不保证前 2 回合必摸到
  - 目标做成“高概率接触”
  - 不加保底抽牌，不做定向检索
...
- 本轮明确 3 种失败路径，并让系统分别对应：
  ...
  - `被敌人克制`
    ...
    - 主要由 Act 2/3 敌人池制造
- 章节体验目标固定：
  - Act 1：乱玩大概率到不了 Boss；正常玩可稳定通过首精英但仍有压力
...
- 新增 simulation 测试：
  ...
  - 多 seed 稳定性验证（至少 5 seeds）：
    - guard / burst / mixed 的 firstEliteWin 波动需在合理区间
    - 防止单 seed 偶然性误导平衡判断
  - 首精英分布验证：
    - 不允许单一精英成为某一路线的绝对主杀手
    - 各精英对不同路线应有分工（executioner/ twin_hunter / debt_monk）
...
- 首精英池已达到可用基线：
  ...
  - 防守流：可玩且不再被单一精英压死
  - 混合流：低但合理
- executioner 顺序调整（heavy 后移）为关键修复点
- guard 问题主要通过“补能力”+“调整出题节奏”联合解决
- 当前策略：
  - 冻结 Act1 首精英与核心卡牌
  - 不再连续微调
  - 后续优先观察多 seed 与后续章节表现
